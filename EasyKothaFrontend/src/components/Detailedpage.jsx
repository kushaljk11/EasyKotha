...
const Detailpage = () => {
  ...
  const handleBooking = async () => {
    ...
    try {
      ...
      toast.error(error.response?.data?.message || "Error sending visit request");
    } finally {
      setBookingLoading(false);
    }
  };
      ...
      <div className="w-full px-4 md:px-6 py-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-2 h-112.5 mb-10 rounded-3xl overflow-hidden">
          ...
          <div className="md:col-span-2 md:row-span-2 relative group cursor-pointer">
            <img 
              src={post.images?.[0] || "https://images.unsplash.com/photo-1502672260266-1c1ef2d93688?q=80&w=2000&auto=format&fit=crop"} 
          ...
          <div className="md:col-span-2 grid grid-cols-2 gap-2 h-full">
            {(post.images?.length > 1 ? post.images.slice(1, 5) : [1,2,3,4]).map((img, idx) => (
              <div key={idx} className="relative group overflow-hidden cursor-pointer bg-gray-50">
                <img 
                  src={typeof img === 'string' ? img : "https://images.unsplash.com/photo-1522708323590-d24dbb6b0267?q=80&w=800&auto=format&fit=crop"} 
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12">
          ...
          <div className="lg:col-span-8">
            ...
            <div className="flex items-center gap-3 mb-3">
              ...
              {post.status === 'approved' && (
                <span className="bg-green-50 text-green-700 text-[10px] font-bold px-2 py-1 rounded uppercase tracking-wider flex items-center gap-1">
                  <CheckCircle2 size={12} className="fill-green-600 text-white" /> Verified Listing
                </span>
              )}
            </div>
            ...
            <div className="flex flex-wrap gap-4 border-t border-b border-gray-100 py-6 mb-8">
              <div className="flex items-center gap-3 pr-6 border-r border-gray-100 last:border-0">
                <div className="p-2 bg-gray-50 rounded-lg text-gray-600"><BedDouble size={20} /></div>
                <div>
                  <p className="text-sm font-bold text-gray-900">{post.beds || 0} Beds</p>
                  <p className="text-xs text-gray-400 capitalize">{post.furnishing || "Unfurnished"}</p>
                </div>
              </div>
              
              <div className="flex items-center gap-3 pr-6 border-r border-gray-100 last:border-0">
                <div className="p-2 bg-gray-50 rounded-lg text-gray-600"><Bath size={20} /></div>
                <div>
                  <p className="text-sm font-bold text-gray-900">{post.baths || 0} Baths</p>
                  <p className="text-xs text-gray-400">Available</p>
                </div>
              </div>

              <div className="flex items-center gap-3 pr-6 border-r border-gray-100 last:border-0">
                <div className="p-2 bg-gray-50 rounded-lg text-gray-600"><UtilityPole size={20} /></div>
                <div>
                  <p className="text-sm font-bold text-gray-900">{post.floor || "Ground"} Floor</p>
                  <p className="text-xs text-gray-400">Position</p>
                </div>
              </div>

              <div className="flex items-center gap-3 pr-6 border-r border-gray-100 last:border-0">
                <div className="p-2 bg-gray-50 rounded-lg text-gray-600"><Store size={20} /></div>
                <div>
                  <p className="text-sm font-bold text-gray-900">{post.parking || "None"} Parking</p>
                  <p className="text-xs text-gray-400">Available</p>
                </div>
              </div>
            </div>
          ...
          <div className="lg:col-span-4 space-y-6 h-fit sticky top-24">
            ...
            <div className="bg-white rounded-3xl p-6 shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-gray-100">
              ...
              {/* Price Header */}
              <div className="mb-6">
                <p className="text-xs text-gray-400 font-medium mb-1">Rent Price</p>
                <div className="flex items-end gap-1 mb-3">
                  <h3 className="text-2xl font-bold text-[#19545c]">NPR {post.price?.toLocaleString() || "0"}</h3>
                  <span className="text-gray-400 text-sm mb-1">/ mo</span>
                </div>
                <div className="flex gap-2">
                  <span className="bg-green-50 text-green-700 text-[10px] font-bold px-2 py-1 rounded">
                    {post.isAvailable ? 'Available Now' : 'Occupied'}
                  </span>
                  <span className="bg-blue-50 text-blue-700 text-[10px] font-bold px-2 py-1 rounded">No Broker Fee</span>
                </div>
              </div>
              ...
              <div className="space-y-4">
                <div>
                  <label className="text-[10px] font-bold text-gray-400 uppercase tracking-wider mb-2 block">Choose a start date</label>
                ...
                <div className="grid grid-cols-1 gap-2">
                  <button 
                    ...
                    className="w-full bg-[#19545c] hover:bg-[#144249] disabled:bg-gray-400 text-white font-semibold uppercase tracking-widest py-4 rounded-xl text-[11px] flex items-center justify-center gap-2 transition-all shadow-lg shadow-[#19545c]/20"
                  >
                    {bookingLoading ? "Processing..." : "Confirm Booking"} <ArrowRight size={14} />
                  </button>
            ...
            <div className="bg-white rounded-2xl p-5 border border-gray-100 flex items-center justify-between shadow-sm">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h4 className="text-sm font-bold text-gray-900">Hosted by {post.author?.name}</h4>
              ...
              >
                ...
                <div className="bg-slate-50 p-4 rounded-2xl border border-dashed border-slate-200 mt-2">
                  <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-2">How it works</p>
                  <ul className="space-y-2">
                    <li className="flex gap-2 items-start text-[11px] text-slate-500">
                      <div className="w-4 h-4 rounded-full bg-[#19545c] text-white flex items-center justify-center text-[8px] mt-0.5 shrink-0">1</div>
                      <span>Select your move-in or visit date.</span>
                    </li>
                    <li className="flex gap-2 items-start text-[11px] text-slate-500">
                      <div className="w-4 h-4 rounded-full bg-[#19545c] text-white flex items-center justify-center text-[8px] mt-0.5 shrink-0">2</div>
                      <span>Landlord receives your request instantly.</span>
                    </li>
                    <li className="flex gap-2 items-start text-[11px] text-slate-500">
                      <div className="w-4 h-4 rounded-full bg-[#19545c] text-white flex items-center justify-center text-[8px] mt-0.5 shrink-0">3</div>
                      <span>Once approved, you'll receive a notification!</span>
                    </li>
                    <li className="flex gap-2 items-start text-[11px] text-slate-500">
                      <div className="w-4 h-4 rounded-full bg-[#19545c] text-white flex items-center justify-center text-[8px] mt-0.5 shrink-0">4</div>
